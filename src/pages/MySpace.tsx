            ...
            {activeTab === 'upcoming' && (
              ...
              >
                {isLoading ? (
                  <>
                    {[...Array(2)].map((_, i) => (
                      <div key={`skel-${i}`} className="bg-white/50 dark:bg-slate-800/50 rounded-2xl p-6 border-2 border-slate-100 dark:border-slate-700 flex flex-col md:flex-row items-center gap-6 animate-pulse">
                        <div className="w-16 h-16 rounded-2xl bg-slate-200 dark:bg-slate-700 shrink-0"></div>
                        <div className="flex-1 w-full text-center md:text-left space-y-3">
                          <div className="h-6 w-3/4 max-w-[200px] bg-slate-200 dark:bg-slate-700 rounded-lg mx-auto md:mx-0"></div>
                          <div className="flex flex-wrap items-center justify-center md:justify-start gap-4">
                        ...
                        <div className="w-full md:w-32 h-12 bg-slate-200 dark:bg-slate-700 rounded-xl"></div>
                      </div>
                    ))}
                  </>
                ) : upcomingLessons.length === 0 ? (
                  <div className="text-center py-16 bg-white/50 dark:bg-slate-800/50 backdrop-blur-xl rounded-[2rem] border-2 border-dashed border-slate-200 dark:border-slate-700">
                    <Calendar className="mx-auto mb-4 text-slate-300 dark:text-slate-600" size={48} />
                    <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2">No upcoming lessons</h3>
                    <p className="text-slate-500 dark:text-slate-400 font-medium">Your schedule is clear. Time to learn something new!</p>
                  </div>
                ) : (
              ...
              >
                {historyLessons.length === 0 ? (
                  <div className="text-center py-16 bg-white/50 dark:bg-slate-800/50 backdrop-blur-xl rounded-[2rem] border-2 border-dashed border-slate-200 dark:border-slate-700">
                    ...
                    <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2">No history yet</h3>
                    <p className="text-slate-500 dark:text-slate-400 font-medium">Completed classes and past skill swaps will appear here.</p>
                  </div>
                ) : (
